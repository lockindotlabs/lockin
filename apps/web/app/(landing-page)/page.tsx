    ...
    <AnimatePresence>
      {showSticky && (
        <motion.div
          ...
          className="fixed top-0 right-0 left-0 z-50 border-b border-neutral-200/50 bg-white/80 px-6 backdrop-blur-md sm:px-8"
        >
...
function HeroSection() {
  ...
  return (
    <div
      id="main-section"
      className="relative flex h-[calc(100dvh-1.5rem)] min-h-[720px] flex-col overflow-hidden rounded-2xl border border-border bg-neutral-100 shadow-md"
    >
      ...
      >
        ...
        <div className="absolute inset-0 bg-white/10 backdrop-blur-[165px]" />
      </div>
        ...
        <div className="flex flex-col items-start gap-8 lg:flex-row lg:justify-between lg:gap-24">
          ...
          >
            <div className="relative overflow-hidden">
              ...
              <div className="pointer-events-none absolute inset-y-0 right-0 w-24 bg-gradient-to-r from-transparent via-white/30 to-white sm:w-36 lg:w-48" />

              <div className="pointer-events-none absolute inset-x-0 bottom-0 w-full bg-gradient-to-b from-transparent via-white/30 to-white sm:h-16 lg:h-24" />
            </div>
          </motion.div>
        </div>
      ...
      <div className="grid grid-cols-1 items-stretch gap-6 md:grid-cols-3">
        {PRICING_PLANS.map((plan, idx) => (
          <motion.div
            key={plan.name}
            {...revealUp(idx * 0.08)}
            className="flex flex-col rounded-2xl border bg-white p-1"
          >
            <div className="flex flex-1 flex-col rounded-xl border bg-white p-5 sm:p-5">
              {/* Plan Header */}
              <div className="mb-5">
              ...
              {/* Features List */}
              <ul className="flex flex-col gap-2 text-sm leading-normal text-neutral-600/90">
  ...
  return (
    ...
    >
      ...
      <motion.div {...revealUp(0.08)}>
        <Accordion className="mr-auto max-w-4xl border-t border-neutral-200/60">
          {FAQ_ITEMS.map((item) => (
            <AccordionItem
              key={item.value}
              value={item.value}
            ...
            >
              <AccordionTrigger className="py-4 text-sm font-medium text-neutral-800 hover:no-underline sm:text-base">
                {item.question}
              </AccordionTrigger>
              <AccordionContent className="pr-4 pb-6 text-sm leading-relaxed text-neutral-600/90 sm:text-base">
                {item.answer}
              </AccordionContent>
            </AccordionItem>
          ))}
            ...
            <ul className="flex flex-col gap-2.5 text-sm">
              {column.links.map((link) => (
                <li key={link.label}>
                  <Link
                    href={link.href}
                    className="inline-flex items-center gap-0.5 text-neutral-600 transition-colors hover:text-neutral-900 hover:underline hover:decoration-dotted"
                  >
                    {link.label}
